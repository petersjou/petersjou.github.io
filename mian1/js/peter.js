...
//判断非空
function nonull(obj){
	obj.attr({'id-data':true});
	var userName=$.trim(obj.val());
	if(userName==''){
		obj.attr({'id-data':false});
		obj.siblings().addClass('false');
	}else{
		obj.attr({'id-data':true});
		obj.siblings().removeClass('false').addClass('true');
	}
}
//键盘按下时清空两个class
function jiance(obj){
	obj.siblings().removeClass('false').removeClass('true')
}
